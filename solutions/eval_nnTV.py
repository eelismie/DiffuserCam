...
def mirflickr_dataset(data, n_files, n_iter, single_psf, save):
    ...
    for fn in files:
        ...
        pds = PDS(dim=H.shape[1], F=F, G=G, H=Hcal, K=grad, verbose=10, max_iter=n_iter, accuracy_threshold=1e-4)

        estimate, converged, diagnostics = pds.iterate()

        est = estimate['primal_variable'].reshape(diffuser_prep.shape)
        est[est < 0] = 0
